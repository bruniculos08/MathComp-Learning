...
(*
  Bruno's note:
    ...
    (1) When using some "reflect_lemma" (like "eqP") the tatic
    "/" used as in "/reflect_lemma" changes the reflect expression
    to a "<->" (bi-implication expression) allowing to rewrite. 

    (2) Note that using "move=> /lemma" will only change stuff in
    the antecedent of the implication goal, but if we pass a name
    for it like "move=> /lemma H" the alterated antecedent will be
    introduced into the context.
*)
...
(* 
  Bruno's note:
    ...
    - using "move=> {}H" instead of "move=> H"
    allows to replacing an existing hypothesis with name
    "H".
*)
by rewrite Hb3 Hb1.
Qed.
